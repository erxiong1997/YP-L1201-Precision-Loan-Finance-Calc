...
class PlfFixedDepositActivity : PlfBindingActivity<ActivityFixedDepositPlfBinding>(
    ...
    override fun beginViewAndDoPlf() {
        ...
        mPlcBinding.tvMonthYear.setSafeListener {
            val popView = PopupWindow(this).apply {
                ...
                val view = LayoutInflater.from(this@PlfFixedDepositActivity)
        ...
        mPlcBinding.tvNumberInvestment.setSafeListener {
            val popView = PopupWindow(this).apply {
                width = mPlcBinding.tvNumberInvestment.width
                height = ViewGroup.LayoutParams.WRAP_CONTENT
                val view = LayoutInflater.from(this@PlfFixedDepositActivity)
                    .inflate(R.layout.item_pop_investment_number_plf, null, false)
                val number1 = view.findViewById<TextView>(R.id.tvNumber1)
                val number4 = view.findViewById<TextView>(R.id.tvNumber4)
                val number12 = view.findViewById<TextView>(R.id.tvNumber12)

                number1.setOnClickListener {
                    dismiss()
                    mNumberInvestment = 1
                    mPlcBinding.tvNumberInvestment.text = getString(R.string.app_1)
                }
                number4.setOnClickListener {
                    dismiss()
                    mNumberInvestment = 4
                    mPlcBinding.tvNumberInvestment.text = getString(R.string.app_4)
                }
                number12.setOnClickListener {
                    dismiss()
                    mNumberInvestment = 12
                    mPlcBinding.tvNumberInvestment.text = getString(R.string.app_12)
                }
                when (mNumberInvestment) {
                    1 -> number1.isSelected = true
                    4 -> number4.isSelected = true
                    12 -> number12.isSelected = true
                }
                contentView = view
                isFocusable = true
                setBackgroundDrawable(0x00000000.toDrawable())
    ...
    private fun calculatorResult() {
        ...
        val dataPersonalLoanPlf = DataPersonalLoanPlf().apply {
            ...
            numberInvestment = mNumberInvestment
        }
    ...
    override fun getLayoutValue(): ActivityFixedDepositPlfBinding {
        return ActivityFixedDepositPlfBinding.inflate(layoutInflater)
    }
}